...
const CreationModal = ({setModalVisible}) => {
  ...
  const nextStage = async () => {
    ...
    if (stage == 0) setQuestion(newText);
    const temp = stage;
    setStage(temp + 1);
    console.log(stage);
    clearInput();
  ...
  const addOption = () => {
    if (options == 0) setO1(newText);
    if (options == 1) setO2(newText);
    if (options == 2) setO3(newText);
    if (options == 3) setO4(newText);
    const temp = options;
    setOptions(temp + 1);
  };
  ...
  const NavSection = () => (
    <View
      ...
      {(stage != 1 || options > 1) &&
        NavButton(nextStage, stage == 3 ? 'Post' : 'Next')}
    </View>
  );
          ...
          <View style={{height: height * 0.255, width, alignItems: 'center'}}>
            <TouchableOpacity
              ...
              {media.m1 && (
                <Image
                  source={{uri: media.m1}}
                  style={{flex: 1, borderRadius: width * 0.077}}
                />
              )}
            </TouchableOpacity>
          </View>
          ...
          <View>
            ...
            <View style={{height: height * 0.191, width, alignItems: 'center'}}>
              ...
              <TextInput
                ...
                placeholder="why do cows not fly?"
              />
            </View>
          </View>
        )}
