...
const CreatePoll = ({setPoll}) => {
  ...
  const screenHeight = dimensions.height - useBottomTabBarHeight();
  -STATUS_BAR;
  ...
  const [comm, selectComm] = useState(null);
    ...
    <View
      ...
      <View style={Top_Options_BG}>
        ...
        <Text style={Header_Text}>
          Create a poll, find out what others think!
        </Text>
        <CreateBar setSection={setSection} ready={ready} />
      </View>
      <View style={Content_Section}>
        {section == 0 ? (
          <Question setQuestionText={setQuestionText} question={questionText} />
        ) : section == 1 ? (
          <Choices setOptions={setOptionsText} optionsText={optionsText} />
        ) : section == 2 ? (
      ...
      <TouchableOpacity
        ...
        <Text
          style={{
            fontSize: 20,
            fontFamily: 'SFRound',
            color: `rgba(131,239,177,${canPost ? 1 : 0.3})`,
          }}>
          POST
        </Text>
      </TouchableOpacity>
