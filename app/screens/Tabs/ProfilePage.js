...
const PVV_Text = ({num, text}) => {
    return (
        <View style={{alignItems: 'center'}}>
            <Text style={{fontWeight: 'bold', fontSize: dimensions.width/20}}>{num}</Text>
            <View style={{height: '5%'}}/>
            <Text style={{fontWeight: 'bold', fontSize: dimensions.width/24, color: '#9c9c9c'}}>{text}</Text>
        </View>
    )
}
    ...
    return (
        <View style={{alignItems: 'center'}}>
            ...
            {!route.params?.user &&
                ...
                size={30}
                color={'black'}
                style={{borderTopWidth: 1, borderTopColor: 'red', position: 'absolute', top: '8%', right: '8%'}}
            />
            }
            ...
            {/* Profile Image */}
            <Image source={{uri: route.params?.user ? `http://${url}${route.params.user.profile.avatar}` : user.profile.avatar}} style={Styles.profilePic}/>

            {/* Profile Name */}
            <Text style={{fontWeight: 'bold', fontSize: dimensions.width/18, padding: '3%', textAlign: 'center'}}>
            ...
            <View style={{flexDirection: 'row', width: dimensions.width, justifyContent: 'space-evenly'}}>
                ...
                <TouchableOpacity
                    onPress={() =>
                    navigation.push(route.params.pollListScreen, { id: route.params?.user ? route.params.user.id : user.id, })}
                >
            ...
            <View style={{width: dimensions.width}}>
                <Text style={{paddingHorizontal: '5%', fontWeight: 'bold', fontSize: dimensions.width/18}}>Popular Polls</Text>
            </View>
            ...
            {polls.length > 0 &&
                <TouchableOpacity>
                <PollCard color={'#51E0B8'} qText={polls[0].question_text} id={polls[0].id}/>
                </TouchableOpacity> 
            }
            {polls.length > 1 && 
