    ...
    <Modal
      ...
      <View style={Modal_Position}>
        <View style={Modal_Container}>
          ...
          <TouchableOpacity
            onPress={() => SetFilter({idx: 4, txt: 'No Filter', cid: 0})}>
          ...
          {post.choices.map((option, idx) => {
            return (
              <TouchableOpacity
                key={idx}
                onPress={() =>
                  SetFilter({idx: idx, txt: option.choice_text, cid: option.id})
                }>
                <View style={[Filter_Button, {borderColor: colors[idx]}]}>
                  <Text style={{fontWeight: 'bold', color: colors[idx]}}>
                    {option.choice_text}
                  </Text>
                </View>
              </TouchableOpacity>
