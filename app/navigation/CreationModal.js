...
const CreationModal = ({setModalVisible}) => {
    ...
    const NavButton = (action, text) => (<Button textSize={15} action={() => action()} style={{marginHorizontal: '2%', width: width*0.359, height: height*0.047, backgroundColor: '#414141', borderRadius: height*0.012}} textColor={'white'} text={text}/> )
    ...
    const NavSection = () => (
        <View style={{height: height*0.047, justifyContent: 'center', flexDirection: 'row', width: width*0.9}}>
            { (stage >= 1) && 
                (
                    (options >= 2) ?
                    <IconButton backgroundColor={'#414141'} action={() => goBack()} style={{ marginHorizontal: '2%', height: height*0.047, backgroundColor: '#414141', borderRadius: 1000}} name={"chevron-back-outline"} iconFill={'white'}/>
                    :
                    NavButton(goBack, 'Back')
                )
            }

            { stage == 3 && NavButton({}, 'Preview') }

            { stage == 1 && options < 4 && NavButton(addOption, 'Add Choice') }

            { (stage != 1 || options > 1) && NavButton(nextStage, stage == 3 ? 'Post' : 'Next') }
        </View>
    )
