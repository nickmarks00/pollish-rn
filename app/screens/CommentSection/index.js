...
const CommentSection = ({route}) => {
  ...
  const adjustFilter = () => {
    const temp = filter;
    if (temp >= route.params.post.choices.length - 1) setFilter(-1);
    ...
    <HideKeyboard>
      <View style={{flex: 1, alignItems: 'center'}}>
        ...
        <View
          ...
          <Button
            ...
            text={
              filter == -1
                ? 'Showing All'
                : '"' + route.params.post.choices[filter].choice_text + '"'
            }
          />
        </View>
        ...
        <KeyboardAvoidingView
          ...
          <View style={Comments_DisplayArea}>
            <ScrollView contentContainerStyle={{alignItems: 'center'}}>
              {comments.map((comment, index) => {
                if (
                  filter == -1 ||
                  route.params.post.choices[filter].id == comment.choice_id
                )
                  return (
                    <View key={index}>
          ...
          <CreateComment reload={fetchDataFromApi} post={post} />
        </KeyboardAvoidingView>
      </View>
    </HideKeyboard>
  );
